...
hist(samples[,1])
```
Calculate what it actually predicts
```{r}
x[i-1] * a + (1 - x[i-1]) * b

```
Now you need to simulate data from this model
```{r}
getSimulatedData <- function(samples){
  simY <- []
  for (row in samples):
    samples[row]$SimY = 
}
...
true_d <- -2
true_mu0 <- 0.6
#This will take one of the 
simPosterior <- function(a, b, c, d, mu_0, T){
  x <- numeric(T)
  y <- numeric(T)
  x1[1] <- rbinom(1, 1, true_mu0)
  y1[1] <- rnorm(1, mean = x[1], sd = 1)
  
  for (i in 2:T) {
    prob <- ifelse(x1[i-1] == 1, true_a, true_b)
    x1[i] <- rbinom(1, 1, prob)
    mu_y <- x1[i] * true_c * cos(pi * i) + (1 - x1[i]) * true_d * cos(pi * i)
    y1[i] <- rnorm(1, mean = mu_y, sd = 1)
  }
  return (x1, y1)
}
```
