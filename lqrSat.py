...
import numpy as np
import pandas as pd
# This creates the satellite LQR Controller Class
    ...
    def propagate(self, duration):
        ...
        # Longitude converted to radians (for initial position)
            ...
            if drift_distance > self.max_drift:
                ...
                current_state = np.array([self.x, self.y, self.z, self.vx, self.vy, self.vz])
                desired_state = np.array([
                    ideal_x, ideal_y, ideal_z,
                    -self.orbital_velocity() * np.sin(angle),
                     self.orbital_velocity() * np.cos(angle),
                     0.0
                ])

                ax, ay, az = self.lqr_control(current_state, desired_state)
