...
def plot3d(log_df, policy):
    ...
    fileName = 'setPolicies/savedPlots/' + policy + '_satOnly3d.png'
    plt.savefig(fileName, dpi=300, bbox_inches='tight')
    plt.show()

def plot2d(log_df, policy):
    """
    ...
    ax.legend()
    plt.tight_layout()
    fileName = 'setPolicies/savedPlots/' + policy + '_satOnly_2d.png'
    plt.savefig(fileName, dpi=300, bbox_inches='tight')
    plt.show()

# ---------- Public plotting API ----------
def plot_paths_xy(df_or_path: Union[str, pd.DataFrame], policy: str,
    ...
    # Save or show
    #if save_path: #WNC -- commented these 2 out b/c I am on collab
        #plt.savefig(save_path, dpi=150, bbox_inches='tight')
    fileName = 'setPolicies/savedPlots/' + policy + '_accel_y.png'
    plt.savefig(fileName, dpi=300, bbox_inches='tight')
    if show:
        plt.show()
    plt.close(fig)

def plot_paths_3d(df_or_path: Union[str, pd.DataFrame], policy: str,
                  earth_radius_km: float = EARTH_RADIUS_KM,
                  save_path: str | None = None,
                  show: bool = True,
                  focus: str = "geo",
    ...
    #if save_path: #WNC -- commented these 2 out b/c I am on collab
        #plt.savefig(save_path, dpi=150, bbox_inches='tight')
    fileName = 'setPolicies/savedPlots/' + policy + '_plot_paths_xy.png'
    plt.savefig(fileName, dpi=300, bbox_inches='tight')
    if show:
        plt.show()
    plt.close(fig)

def plot_error(df, policy):
  distance_from_ideal = ((df['X (km)'] - df['ideal x'])**2 + (df['Y (km)'] - df['ideal_y'])**2 + (df['Z (km)'] - df['ideal_z'])**2)**0.5

  plt.figure(figsize=(12, 6))
  plt.plot(df['Time (s)'], distance_from_ideal)
  plt.xlabel('Time (s)')
  plt.ylabel('Distance from Ideal Position (km)')
  plt.title('Satellite Distance from Ideal Position Over Time')
  plt.grid(True)
  fileName = 'setPolicies/savedPlots/' + policy + '_plot_error.png'
  plt.savefig(fileName, dpi=300, bbox_inches='tight')
  plt.show()

def plot_accel_x(df, policy):
  plt.figure(figsize=(12, 6))
  #plt.plot(df['Time (s)'].head(60), df['Acceleration(x)'].head(60))
  plt.plot(df['Time (s)']-1, df['Acceleration(x)'])
  plt.xlabel('Time (s)')
  plt.ylabel('Acceleration(x) (km/s^2)')
  plt.title('Satellite Acceleration in X Direction Over Time')
  plt.grid(True)
  fileName = 'setPolicies/savedPlots/' + policy + '_accel_x.png'
  plt.savefig(fileName, dpi=300, bbox_inches='tight')
  plt.show()

def plot_accel_y(df, policy):
  plt.figure(figsize=(12, 6))
  #plt.plot(df['Time (s)'].head(60), df['Acceleration(x)'].head(60))
  plt.plot(df['Time (s)']-1, df['Acceleration(y)'])
  plt.xlabel('Time (s)')
  plt.ylabel('Acceleration(y) (km/s^2)')
  plt.title('Satellite Acceleration in Y Direction Over Time')
  plt.grid(True)
  fileName = 'setPolicies/savedPlots/' + policy + '_accel_y.png'
  plt.savefig(fileName, dpi=300, bbox_inches='tight')
  plt.show()

def plot_accel_z(df, policy):
  plt.figure(figsize=(12, 6))
  #plt.plot(df['Time (s)'].head(60), df['Acceleration(x)'].head(60))
  plt.plot(df['Time (s)']-1, df['Acceleration(z)'])
  plt.xlabel('Time (s)')
  plt.ylabel('Acceleration(z) (km/s^2)')
  plt.title('Satellite Acceleration in Z Direction Over Time')
  plt.grid(True)
  fileName = 'setPolicies/savedPlots/' + policy + '_accel_z.png'
  plt.savefig(fileName, dpi=300, bbox_inches='tight')
  plt.show()

def allPlots(df, policy):
    plot_paths_xy(df, policy= policy, focus="geo")
    plot_paths_3d(df, policy = policy, focus="geo")

    # 2) If you want the full system including the Sun (everything in one frame)
    plot_paths_xy(df, policy = policy, focus="all")
    #plot_paths_3d(df, policy = policy, focus="all")

    # 3) just plots the satellite around earth station keeping vs not station keeping
    plot2d(df, policy)
    plot3d(df, policy)

    # 4) Plot the error from the desired position over time
    plot_error(df, policy)

    #5) plot accel
    plot_accel_x(df, policy)
    #plot_accel_y(df)
    plot_accel_z(df, policy)